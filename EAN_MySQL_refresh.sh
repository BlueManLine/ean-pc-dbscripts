...
do
    ...
    if [ "$ZIPOUT" -eq 0 ] && [ "$CHKSUM_PREV" != "$CHKSUM_NOW" ] && [ "$records" -gt 0 ] && [ -s ${FILE}.txt ] && [ -r ${FILE}.txt ]; then
        ...
        if [ $tablename = "activepropertybusinessmodel" ]; then
			...
			$CMDSP_MYSQL --execute="CALL sp_log_createcopy();"
        ...
        if [ $tablename = "activepropertybusinessmodel" ]; then
			...
			# for version 5.6.6+ you will need this line instead to use stored credentials
			#CMDSP_MYSQL="${MYSQL_DIR}mysql --login-path=${MYSQL_LOGINPATH} --default-character-set=utf8 --protocol=${MYSQL_PROTOCOL} --port=${MYSQL_PORT} --database=${MYSQL_DB}"
			$CMDSP_MYSQL --execute="CALL sp_log_addedrecords();"
			$CMDSP_MYSQL --execute="CALL sp_log_erasedrecords();"
			$CMDSP_MYSQL --execute="CALL sp_log_erase_common();"
			$CMDSP_MYSQL --execute="CALL sp_log_erase_deleted();"
			$CMDSP_MYSQL --execute="CALL sp_log_changedrecords();"
...
### Run stored procedures as required for extra functionality       ###
### you can use this section for your own stuff                     ###
CMD_MYSQL="${MYSQL_DIR}mysql  --default-character-set=utf8 --protocol=${MYSQL_PROTOCOL} --port=${MYSQL_PORT} --user=${MYSQL_USER} --password=${MYSQL_PASS} --host=${MYSQL_HOST} --database=${MYSQL_EXTRADB}"
# for version 5.6.6+ you will need this line instead to use stored credentials
#CMD_MYSQL="${MYSQL_DIR}mysql --login-path=${MYSQL_LOGINPATH} --default-character-set=utf8 --protocol=${MYSQL_PROTOCOL} --port=${MYSQL_PORT} --database=${MYSQL_EXTRADB}"
#$CMD_MYSQL --execute="CALL sp_fill_fasttextsearch();"
